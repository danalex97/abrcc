...
def main():
	...
	schemes=Set()
	for logfile in args.testcases_logs_qual:
	...
	for log_file in args.testcases_logs_qual:
		...
		time_ms = np.array(time_ms)
		time_all_qual[name] = time_ms
		qual_all[name] = qual

	time_start = min(min(time_all_qual[s]) for s in NAMES_qual)
	for s in NAMES_qual:
		time_all_qual[s] -= time_start
	...
	#plot determined bandwidth
	ax = fig.add_subplot(311)

	for scheme in NAMES_qual:
		ax.plot(time_all_qual[scheme][:], qual_all[scheme][:], label=str(scheme), marker = '+', color = colors[scheme])


	#ax.axhline(y=0, xmin=0, xmax=1, color='black')
	plt.ylabel('determined quality (Mbps)')
	filename=args.testcases_logs[0].split('/')[-1]
	title=filename.split('_')[1]
	plt.title(title+'\nruns: '+str(get_run_numbers(args.testcases_logs)))

	tcp_end = []
	filename_tcp=args.testcases_logs[0].split('/')[0] + '/' + args.testcases_logs[0].split('/')[1] + '/' + args.testcases_logs[0].split('/')[2]
	print(filename_tcp)
	filename_tcp = filename_tcp + '/tcp.txt'
	f = open(filename_tcp, "r")
	for x in f:
  		tcp_end.append(float(x))
	tcp_start = [20.491921734377712, 0.35838527143778504, 30.07350136756547, 101.249258959991, 6.963782080923449, 27.89669421727359] 
	for xc in tcp_start:
	    plt.axvline(x=xc, color='y', linestyle='--')
	for i in range(0, len(tcp_start)):
	    plt.axvline(x=tcp_start[i]+tcp_end[i], color='tab:gray', linestyle='--')

	# plot bitrate decisions
	ax2 = fig.add_subplot(312, sharex=ax)
	# ax.set_xlabel('time (s)')
	ax2.get_yaxis().set_ticks([300,750,1200,1850,2850,4300]) #bitrates of testvideo

	for scheme in NAMES:
		ax2.plot(time_all[scheme][:], bit_rate_all[scheme][:], label=scheme,drawstyle='steps-pre', marker='.', color=colors[scheme])
	...
	for scheme in NAMES:
		ax3.plot(time_all[scheme][:], buff_all[scheme][:], label=str(scheme), marker = '+', color = colors[scheme])

	# plot buffer drain visualisation
	for scheme in NAMES:
		ax3.plot(buffer_drain_time[scheme][:], buffer_drain_blevel[scheme][:], linestyle=':', color=colors[scheme])

	ax3.axhline(y=0, xmin=0, xmax=1, color='black')
...
def read_cmd_arguments():
	...
	num = (len(sys.argv)-1) //2
	#parser.add_argument('--testcase_config', help='path to json file with the testcase information', type=str)
	parser.add_argument('testcases_logs', help='testcase logfiles (logfiles of dash-localhost-server)', nargs=num, type=str)
	parser.add_argument('testcases_logs_qual', help='testcase logfiles (logfiles of dash-localhost-server)', nargs=num, type=str)
