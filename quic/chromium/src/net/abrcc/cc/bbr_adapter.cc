...
QuicRoundTripCount BbrAdapter::BbrInterface::kBandwidthWindowSize() {
...
BbrAdapter::DebugState::DebugState(const DebugState& state) = default;

BbrAdapter::BbrAdapter(QuicTime now,
...
bool BbrAdapter::InSlowStart() const {
  return mode_ == STARTUP;
}

void BbrAdapter::OnPacketSent(QuicTime sent_time,
...
bool BbrAdapter::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth BbrAdapter::PacingRate(QuicByteCount /*bytes_in_flight*/) const {
...
QuicBandwidth BbrAdapter::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount BbrAdapter::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount BbrAdapter::GetSlowStartThreshold() const {
  return 0;
}

bool BbrAdapter::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool BbrAdapter::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool BbrAdapter::IsPipeSufficientlyFull() const {
...
void BbrAdapter::SetFromConfig(const QuicConfig& config,
...
void BbrAdapter::AdjustNetworkParameters(const NetworkParams& params) {
...
void BbrAdapter::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType BbrAdapter::GetCongestionControlType() const {
  return kAbbr;
}

QuicTime::Delta BbrAdapter::GetMinRtt() const {
  return !min_rtt_.IsZero() ? min_rtt_ : rtt_stats_->initial_rtt();
}

QuicByteCount BbrAdapter::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount BbrAdapter::ProbeRttCongestionWindow() const {
...
void BbrAdapter::EnterStartupMode(QuicTime now) {
  if (stats_) {
...
void BbrAdapter::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool BbrAdapter::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool BbrAdapter::UpdateBandwidthAndMinRtt(
    QuicTime now,
    const AckedPacketVector& acked_packets) {
...
bool BbrAdapter::ShouldExtendMinRttExpiry() const {
...
void BbrAdapter::UpdateGainCyclePhase(QuicTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void BbrAdapter::CheckIfFullBandwidthReached() {
...
void BbrAdapter::OnExitStartup(QuicTime now) {
...
void BbrAdapter::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
void BbrAdapter::CalculatePacingRate() {
...
void BbrAdapter::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void BbrAdapter::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string BbrAdapter::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void BbrAdapter::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
BbrAdapter::DebugState BbrAdapter::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(BbrAdapter::Mode mode) {
  switch (mode) {
    case BbrAdapter::STARTUP:
      return "STARTUP";
    case BbrAdapter::DRAIN:
      return "DRAIN";
    case BbrAdapter::PROBE_BW:
      return "PROBE_BW";
    case BbrAdapter::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const BbrAdapter::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const BbrAdapter::DebugState& state) {
  ...
  if (state.mode == BbrAdapter::STARTUP) {
