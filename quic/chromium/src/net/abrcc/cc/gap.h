#ifndef ABRCC_CC_GAP_H_
#define ABRCC_CC_GAP_H_
...
class QUIC_EXPORT_PRIVATE BbrGap : public SendAlgorithmInterface {
 public:
  class __attribute__((packed)) BbrInterface {
   ...
   private:
    ...
    mutable QuicMutex recovery_mutex_;
  ...
  struct QUIC_EXPORT_PRIVATE DebugState {
    explicit DebugState(const BbrGap& sender);
  ...
  BbrGap(const BbrGap&) = delete;
  BbrGap& operator=(const BbrGap&) = delete;
  ~BbrGap() override;
 ...
 private:
  ...
  std::unique_ptr<structs::MovingAverage<double>> bandwidth_estimator;

  // estimator trace
  std::vector<double> estimator_trace;
...
QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os,
                                             const BbrGap::Mode& mode);
QUIC_EXPORT_PRIVATE std::ostream& operator<<(
    std::ostream& os,
    const BbrGap::DebugState& state);
