...
QuicRoundTripCount BbrTarget::BbrInterface::kBandwidthWindowSize() {
  return getGainCycleLength() + 2;
}

void BbrTarget::BbrInterface::setParent(BbrTarget *parent) {
  this->parent = parent;
}
...
void BbrTarget::changeMode(BbrTarget::Mode newMode) {
...
BbrTarget::DebugState::DebugState(const BbrTarget& sender)
    : mode(sender.mode_),
...
BbrTarget::DebugState::DebugState(const DebugState& state) = default;

BbrTarget::BbrTarget(QuicTime now,
...
void BbrTarget::SetInitialCongestionWindowInPackets(
...
bool BbrTarget::InSlowStart() const {
  return mode_ == STARTUP;
}

void BbrTarget::OnPacketSent(QuicTime sent_time,
...
bool BbrTarget::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth BbrTarget::PacingRate(QuicByteCount /*bytes_in_flight*/) const {
...
QuicBandwidth BbrTarget::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount BbrTarget::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount BbrTarget::GetSlowStartThreshold() const {
  return 0;
}

bool BbrTarget::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool BbrTarget::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool BbrTarget::IsPipeSufficientlyFull() const {
...
void BbrTarget::SetFromConfig(const QuicConfig& config,
...
void BbrTarget::AdjustNetworkParameters(const NetworkParams& params) {
...
void BbrTarget::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType BbrTarget::GetCongestionControlType() const {
  return kAbbr;
}

QuicTime::Delta BbrTarget::GetMinRtt() const {
...
QuicByteCount BbrTarget::ProbeRttCongestionWindow() const {
...
void BbrTarget::EnterStartupMode(QuicTime now) {
  if (stats_) {
...
void BbrTarget::EnterProbeBandwidthMode(QuicTime now) {
  changeMode(PROBE_BW);
...
void BbrTarget::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool BbrTarget::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool BbrTarget::ShouldExtendMinRttExpiry() const {
...
void BbrTarget::UpdateGainCyclePhase(QuicTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
  ...
  if (should_advance_gain_cycling) {
    ...
    if (drain_to_target_ && pacing_gain_ < 1 &&
        interface->getPacingGainCycle()[cycle_current_offset_] == 1 &&
...
void BbrTarget::CheckIfFullBandwidthReached() {
...
void BbrTarget::MaybeExitStartupOrDrain(QuicTime now) {
...
void BbrTarget::OnExitStartup(QuicTime now) {
...
void BbrTarget::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
void BbrTarget::CalculatePacingRate() {
...
void BbrTarget::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void BbrTarget::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string BbrTarget::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void BbrTarget::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
BbrTarget::DebugState BbrTarget::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(BbrTarget::Mode mode) {
  switch (mode) {
    case BbrTarget::STARTUP:
      return "STARTUP";
    case BbrTarget::DRAIN:
      return "DRAIN";
    case BbrTarget::PROBE_BW:
      return "PROBE_BW";
    case BbrTarget::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const BbrTarget::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const BbrTarget::DebugState& state) {
  ...
  if (state.mode == BbrTarget::STARTUP) {
